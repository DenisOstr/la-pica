<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-semibold">{{ pizza?.name }} <span class="text-sm text-green-500">({{ pizza?.price }}€)</span></h1>
                <p class="text-sm text-gray-500">Update or delete pizza</p>
            </div>

            <div class="flex space-x-2">
                <BaseButton @click="() => {}">Save</BaseButton>
                <BaseButton color="error" @click="() => {}">Delete</BaseButton>
            </div>
        </div>
    ...
    >
        <div class="p-4 space-y-4">
            ...
            <div class="bg-gray-100 w-full p-2 max-h-52 rounded-md overflow-auto" v-if="allIngredients.length != 0">
                <div
